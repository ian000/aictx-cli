...
- **核心痛点解决**: 传统开发团队在迈入 AI 编程阶段时，必定会遭遇“大模型自由发挥导致架构崩塌”的阵痛。无数团队为了让 AI 写出符合公司既有架构规范、API 风格和业务边界的代码，付出了巨大的试错成本（反复修改 Prompt、反复 Review 烂代码、甚至因为 AI 幻觉引发线上事故）。本项目旨在解决这一团队技术架构刚需：**“如何让团队里的 10 个甚至 100 个 AI 助手遵循同一种架构设计和业务红线，在降低 Token 消耗的同时，从根源上降低 AI 幻觉”**。
- **商业化路径**:
  - **第一阶段**: 开源 CLI 核心 `aictx`，定义全行业通用的“AI 项目 Context as Code 标准目录”。通过解决“团队 AI 架构一致性”这一最痛的刚需，占领开发者社区心智。
  - **第二阶段**: 建立 Rule-Hub（AI 架构规则与技能分发市场），允许顶尖架构师共享针对特定技术栈的优质配置包。
  - **第三阶段**: 面向中大型研发团队，提供企业级私有化 Context 治理平台 (SaaS)，实现公司技术宪法的可视化管理、监控各业务线 AI 规范的同步率与架构 Drift（漂移）情况。向进行 AI 转型的企业“卖护城河”。

## 2. 存量项目接管策略 (Brownfield Onboarding Strategy)
对于没有 `documents/` 知识库的庞大存量项目（占市场 90%），aictx 放弃了昂贵且易产生幻觉的大模型提取方案，转而采用 **“基于 Graphify (AST) 的确定性零模型提取方案”**：
- **核心逻辑**: aictx-cli 静默调用底层 `graphify` 引擎，通过 Tree-sitter 在纯本地、零网络请求的情况下，遍历源码并提取出 100% 真实的物理调用图谱 (Call Graph) 和上帝节点 (God Nodes)。
- **核心优势**: 
  - **零 Token 成本**: 彻底告别大模型 API 账单。
  - **绝对隐私合规**: 源码数据不上传、不离开本地，完美符合中大型企业的 SecOps 安全红线。
  - **极速解析**: 10万行的屎山代码，几秒钟内即可梳理出核心业务边界并生成 `documents/architecture/system-graph.md` 规范。
...
### 3.7 规范驱动的工作流拦截机制 (Spec-Driven Workflow Interceptor)
- **业务痛点**: AI 助手在接到用户需求后，往往急于写代码（毛躁），导致写出的代码与全局架构或业务红线冲突。虽然市面上有 OpenSpec 等工具，但它们采用“用完即归档”的战术级管理，会导致知识库碎片化，无法形成全局统一的 SSOT（单一事实来源）。
- **解决方案 (Context-Aware Planning)**: 吸收 Spec-Driven 的优秀理念，将其升级为“全局上下文感知的拦截器”。
  - **意图拦截 (`aictx plan`)**: 当用户提出新需求时，AI 必须先生成一份临时变更计划（包含：本次修改涉及哪些全局 RAG 文档更新、Schema 变更预测、以及具体的任务拆解）。
  - **人类确认 (Human-in-the-loop)**: 开发者 Review 变更计划，确保设计方向符合业务全局利益。
  - **执行与防腐同步 (`aictx apply`)**: 确认后，AI 执行代码变更，**同时强制更新全局的 `documents/` 知识库和 `schema.prisma`**。我们不“归档（Archive）”临时文件，而是将变更固化到全局数字资产中，实现真正的 Context 迭代防腐。
