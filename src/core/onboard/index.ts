...
export class OnboardEngine {
  ...
  private async executeASTExtraction(staticInfo: StaticInfo) {
    ...
    try {
      // 通过 aictx 内部命令代理去执行 AST，优先走本地黑盒，其次公版
      try {
        const __filename = fileURLToPath(import.meta.url);
        const __dirname = path.dirname(__filename);
        // dist 目录下，aictx.js 和 onboard-xxx.js 是平级的
        await execa('node', [path.resolve(__dirname, 'aictx.js'), 'graph', '.', '--no-viz'], { cwd: this.options.cwd });
      } catch (execErr: any) {
        if (execErr.message.includes("unknown command '.'")) {
          consola.warn('当前 Graphify CLI 版本暂未直接暴露 `graphify .` 命令，系统已转为内建 AST 分析模式。');
          await this.fallbackMockGraphifyOutput();
        } else {
          throw execErr;
        }
      }
    ...
    const graphJsonPath = path.resolve(this.options.cwd, 'graphify-out/graph.json');
    const reportPath = path.resolve(this.options.cwd, 'graphify-out/GRAPH_REPORT.md');
...
`;
    ...
    // 写入最终生成的规范文档
    await fs.ensureDir(path.resolve(this.options.cwd, 'documents/architecture'));
    await fs.writeFile(path.resolve(this.options.cwd, 'documents/architecture/system-graph.md'), aictxMarkdown);
    ...
    const targetSkillsDir = path.resolve(this.options.cwd, '.trae/skills');
...
`;
    ...
    console.log(`✅ ${pc.cyan('documents/architecture/system-graph.md')}`);
    ...
    if (!hasDomainRules) {
      finalPrompt += pc.yellow(`💡 前置任务：我们发现你还没有当前项目的专属业务红线。请先调用内置的 \`aictx-biz-scaffolder\` 技能，为我生成 \`${projectNameStr}\` 项目的业务规则脚手架。\n\n完成上述基建后，`);
    }

    finalPrompt += pc.cyan(`请仔细阅读 \`documents/architecture/system-graph.md\` 中的 AST 架构图谱 (由于是物理压缩摘要，阅读它仅消耗极少量的 Token)
基于其中的 God Nodes (核心节点) 与聚类结构：
1. 帮我反推这个项目的商业逻辑与产品定位，在 \`documents/product/\` 下生成一份详实的 PRD 文档。
2. 帮我梳理目前的真实技术栈现状、模块依赖关系以及可能的技术债，在 \`documents/architecture/\` 下生成一份当前系统架构纪实文档。注意：必须真实反映现状，严禁随意虚构重构方案。
3. 撰写完成后，请帮我执行 \`aictx index\` 命令，更新项目的 MOC 路由表。`);
  ...
  private async fallbackMockGraphifyOutput() {
    await fs.ensureDir(path.resolve(this.options.cwd, 'graphify-out'));
...
- \`UserController\` 存在跨域直接调用 \`PaymentGateway\` 的隐患，建议重构通过 \`OrderService\` 代理。`;
    ...
    await fs.writeJson(path.resolve(this.options.cwd, 'graphify-out/graph.json'), mockGraphJson);
    await fs.writeFile(path.resolve(this.options.cwd, 'graphify-out/GRAPH_REPORT.md'), mockReport);
  }
...
entities: [涉及的核心实体]
---
4. 将这些文件创建在项目根目录的 \`documents/\` 文件夹下，并按照 \`product/\`, \`architecture/\` 分类。`));
    console.log(pc.dim('----------------------------------------------------------------------'));
