...
export class OnboardEngine {
  ...
  private async executeASTExtraction(staticInfo: StaticInfo) {
    ...
    try {
      ...
      const output = await graphify(['-dir', this.options.cwd, '-out', path.resolve(this.options.cwd, 'graphify-out')]);
    ...
    const reportPath = path.resolve(this.options.cwd, 'graphify-out/system-graph.md');
...
This project has been onboarded with \`aictx\` and has a local AST knowledge graph generated by \`graphify-go\` at \`graphify-out/graph.json\`.
