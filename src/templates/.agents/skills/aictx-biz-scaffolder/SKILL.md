...
description: "快速初始化新外包业务的 Codex 规则模板文件。当用户要求开启新项目、创建新业务目录或生成 project-xxx-*.md 规则文件时调用此技能。"
---

# 新业务规则脚手架 (Biz Scaffolder)

当用户接手新的外包项目（如医美 `medical`，仓储 `wms` 等），并要求在 `.agents/workflows/` 下创建新项目的规则时，你必须主动调用此技能，为其生成标准的四大架构文件。

## 工作流 (Workflow)

1. **获取项目标识**: 从用户的请求中提取新项目的英文简写（如 `medical`）。如果用户未提供，则向用户询问。
2. **生成四大核心文件**: 必须使用当前可用的文件编辑工具，在 `.agents/workflows/` 目录下一次性生成以下四个 `.md` 规则文件。
