...
1. **确认文档路径**：主动询问使用者（开发者）他们的产品文档和技术文档所在的具体目录路径（不要假设固定在某个文件夹）。
2. **更新产品需求文档 (PRD)**：在使用者指定的路径下明确业务目标、核心价值和功能模块。
3. **更新技术架构设计**：在使用者指定的路径下补充或修改技术方案、命令设计和模块划分。
...
1. **搜索受影响文档**: 询问开发者相关文档存放的路径，并使用工具在该目录下检索与本次代码变更相关的关键字（如状态名、字段名、公式名）。
...
- 当业务需求发生根本性变更时，在编写代码前，必须先向使用者确认对应的产品/技术文档目录，并在其下创建或更新文档，打上正确的 Obsidian YAML 标签（如 `tags`, `entities`）。
