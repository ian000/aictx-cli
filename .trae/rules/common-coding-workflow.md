...
1. **自动定位文档**：使用 `SearchCodebase` 或 `Glob` 工具全局检索项目中的产品文档 (PRD) 和技术架构文档，而不是反复询问使用者。
2. **更新产品需求文档 (PRD)**：在检索到的产品文档路径下明确业务目标、核心价值和功能模块。
3. **更新技术架构设计**：在检索到的技术架构文档路径下补充或修改技术方案、命令设计和模块划分。
...
1. **自动检索受影响文档**: 使用全局搜索工具 (如 `SearchCodebase`) 在整个项目中检索与本次代码变更相关的关键字（如状态名、字段名、公式名）。
...
- 当业务需求发生根本性变更时，在编写代码前，必须先自动检索并定位项目原有的产品/技术文档目录，并在其下创建或更新文档，打上正确的 Obsidian YAML 标签（如 `tags`, `entities`）。
